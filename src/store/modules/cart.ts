...
const actions = {
	...
	async createOrderAPI(
		...
		console.log('order info is', order_info)
		const line_items = context.state.cart.line_items
		const user_access_token = context.rootState.authorization.access_token
		const session_id = localStorage.getItem("session_id")
		
		const response = await CartDataService.createOrderAdmin(
			line_items, user_access_token, session_id, order_info 
		)
		if (response && response.status == 200) {
